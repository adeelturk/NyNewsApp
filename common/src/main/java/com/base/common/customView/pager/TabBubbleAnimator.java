...
/**
 * This class is a Custom ViewPager with bubble jump animation
 */
public class TabBubbleAnimator extends ViewPager.SimpleOnPageChangeListener {
    ...
    private void getTabView(int position, TabLayout.Tab tab, boolean isSelected) {
        ...
        if(tabLayout.getContext().getResources().getBoolean(R.bool.is_grid)){
            TextViewCompat.setTextAppearance(tabTextView, android.R.style.TextAppearance_Large);
        }
        tabTextView.setTextColor(ContextCompat.getColor(tabTextView.getContext(),R.color.white));
    ...
    private void resetTabView(int position, TabLayout.Tab tab, boolean isSelected) {
        View view = tab.getCustomView() == null ? LayoutInflater.from(tabLayout.getContext()).inflate(R.layout.custom_tab, null) : tab.getCustomView();
        if (tab.getCustomView() == null) {
            tab.setCustomView(view);
        }
